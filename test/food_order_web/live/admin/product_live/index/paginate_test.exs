defmodule FoodOrderWeb.Admin.ProductLive.Index.PaginateTest do
  ...
  describe "paginate component" do
    ...
    test "clicking next, preview, and page", %{conn: conn} do
      ...
      assert_patched(view, ~p"/admin/products?name=&page=2&per_page=4&sort_by=updated_at&sort_order=desc")

      view
      |> element("[data-role=previous]")
      |> render_click()

      assert_patched(view, ~p"/admin/products?name=&page=1&per_page=4&sort_by=updated_at&sort_order=desc")
    ...
    test "using params", %{conn: conn} do
      ...
      {:ok, view, _html} = live(conn, ~p"/admin/products?name=&page=1&per_page=1&sort_by=inserted_at&sort_order=desc")

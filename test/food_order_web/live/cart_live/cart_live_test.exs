defmodule FoodOrderWeb.CartLiveTest do
  ...
  test "load main elements when cart is empty", %{conn: conn} do
    ...
    assert has_element?(view, "[data-role=cart]>div>p", "You probably haven`t ordered a food yet.")
    assert has_element?(view, "[data-role=cart]>div>p", "To order an good food, go to the main page.")

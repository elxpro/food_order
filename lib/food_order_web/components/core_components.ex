defmodule FoodOrderWeb.CoreComponents do
          ...
          <tr>
            ...
            <%= if col[:link] do %>
              <div class="flex items-center">
                ...
                <.link patch={create_link(col[:link], col[:options], col[:label])}>
                <%= if col[:options][:sort_order] == :asc do %>
                  <Heroicons.bars_arrow_up solid class={[Atom.to_string(col[:options][:sort_by]) == col[:label]&& "text-orange-500" || "text-gray-500", "h-5 w-5 stroke-current"]} />
                <% else %>
                  <Heroicons.bars_arrow_down solid class={[Atom.to_string(col[:options][:sort_by]) == col[:label] && "text-orange-500" || "text-gray-500", "h-5 w-5 stroke-current"]} />
                <% end %>
                </.link>
              </div>
            <% else %>
  ...
  defp create_link(url, params, label) do
